...
def build_vocab(text):
    ...
    voc=[text[i] for i in range(len(text))]
    voc=list(set(voc))
    voc.sort()
    return voc
    pass
...
def build_stoi(vocab):
    ...
    char2idx={e:i for i,e in enumerate(vocab)}
    return char2idx
    pass
...
def build_itos(vocab):
    ...
    rev={i:e for i,e in enumerate(vocab)}
    return rev
    pass
...
def encode_string(text, stoi):
    ...
    res=[stoi[text[i]] for  i in range(len(text))]
    return res
    pass
...
def softmax_overflow_demo(large_value):
    ...
    res=array_exp(large_value)
    if res==float("inf"):
        return {'naive_exp': res, 'overflowed': True}
    else:
        return {'naive_exp': res, 'overflowed': False}
...
def stable_softmax_1d(logits):
    ...
    mx=np.max(logits)
    stb=logits-mx
    res=np.exp(stb)/np.sum(np.exp(stb))
    return res
...
def stable_softmax_2d_rowwise(logits):
    ...
    mx=max_along_axis(logits,axis=1)[:, None]
    logits-=mx
    res=array_exp(logits)/sum_keepdims(array_exp(logits),1)
    return res
...
def allocate_count_matrix(vocab_size):
    ...
    res=make_2d_zeros(vocab_size,vocab_size)
...
def vectorize_counts_add_at(vocab_size, data):
    ...
    res=allocate_count_matrix(vocab_size)
    np.add.at(res,(data[:-1], data[1:]),1)
    return res
...
def normalize_counts_to_probs(n_matrix):
    ...
    sums=row_sums_of_counts(n_matrix)
    res=n_matrix/sums
    return res
...
def generate_sequence(p_matrix, start_id, length, rng):
    ...
    for _ in range(length-1):
        next_token=sample_next_token(p_matrix,res[-1], rng)
...
def average_nll(p_matrix, data):
    ...
    res=sum_negative_log_probs(p_matrix, data)/(len(data)-1)
    return res
...
def one_hot_encode_batch(ids, vocab_size):
    ...
    res = np.zeros((len(ids), vocab_size), dtype=np.float64)
...
def observe_lookup_equivalence(w, ids):
    ...
    d={}
    d["onehot_result"]=one_hot @ w
    d["index_result"]=w[ids]
    return d
...
def cross_entropy_loss(probs, targets):
    ...
    loss=-array_log(gather_correct_token_probs(probs,targets))
    return np.mean(loss)
...
def derive_dlogits_on_paper():
    ...
    return (
        ...
        "For one example, L = -sum_j y_j log(probs_j). "
        "Using the softmax derivative and chain rule, "
        "dL/dlogits = probs - onehot(targets). "
        "For mean cross-entropy over B examples, "
...
def compute_dw_scatter_add(ids, dlogits, vocab_size):
    ...
    dW = np.zeros((vocab_size, dlogits.shape[1]), dtype=dlogits.dtype)
...
def sample_from_neural_bigram(w, start_id, num_tokens, itos):
    ...
    for _ in range(num_tokens):
        logits = forward_logits_lookup(w, np.array([res[-1]]))
        probs = logits_to_probs_rowwise(logits)
        next_token = rng.choice(w.shape[0], p=probs[0])
        res.append(next_token)
    return decode_ids(res, itos)
...
def layernorm_forward_variance(x, mean):
    ...
    res = np.mean((x - mean) ** 2, axis=-1, keepdims=True)
    return res
...
def layernorm_backward_full(dy, cache):
    ...
    dx = (inv_std / D*(
            ...
            - x_hat
            * np.sum(dx_hat * x_hat, axis=-1, keepdims=True)
        )
    )
...
def create_token_embedding(vocab_size, d_model, scale=0.02):
    ...
    np.random.seed(0)
    emb=np.random.randn(vocab_size,d_model).astype(np.float64)
    emb*=scale
    return emb
...
def create_positional_embedding(block_size, d_model, scale=0.02):
    ...
    pos_emb=make_2d_random(block_size,d_model,0)
    res=scale_w_small(pos_emb,scale)
    return res
...
def embedding_sum_backward(d_out):
    ...
    _,T,d_model=d_out.shape
    d_pos_emb=sum_axis0(d_out)
    cache={"d_token_emb":d_out,"d_pos_emb":d_pos_emb}
    return cache
...
def apply_causal_mask(scaled_scores, causal_mask):
    ...
    res=np.where(causal_mask==False,float("-inf"),scaled_scores)
    return res
...
def softmax_attention_weights(masked_scores):
    ...
    mx=np.max(masked_scores, axis=-1, keepdims=True)
    res=masked_scores-mx
    soft=np.exp(res)/np.sum(np.exp(res),axis=-1,keepdims=True)
    return soft
...
def output_projection_backward(d_proj, cache):
    ...
    dw_o= attn_out.reshape(B*T,d_head).T @ d_proj.reshape(B*T,d_proj.shape[-1])
    res={"dw_o":dw_o,"d_attn_out":d_attn_out}
    return res
